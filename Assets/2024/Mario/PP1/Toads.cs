...
public class Toads : MonoBehaviour
{
    [SerializeField]
    ...
    void Update()
    {
        ...
        if(Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Ended)
        {
            ...
            {
                ThrowToad(Input.GetTouch(0).position);
            }
        }
    ...
    void ThrowToad(Vector2 mousePos)
    {
        _rb.useGravity = true;
        ...
        _rb.AddForce((direction * speed / 2f) + (Vector3.up * speed));
        holding = false;
        thrown = true;
    ...
    {
        if(collision.transform.tag == "Piranha")
        {
            Debug.Log("Dans le mile !");
        }
    }
}